  ...
  return <section className="py-24 bg-gradient-subtle">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-16">
          {[1, 2, 3, 4, 5, 6, 7, 8].map(index => <Card key={index} className="aspect-square overflow-hidden group animate-in fade-in scale-in duration-700" style={{
        ...
        }].map((testimonial, index) => <Card key={index} style={{
          animationDelay: `${(index + 8) * 100}ms`
        }} className="p-6 animate-in fade-in slide-in-from-bottom duration-700 bg-gray-900">
              ...
              <div className="flex gap-1 mb-3">
                {[...Array(5)].map((_, i) => <span key={i} className={i < testimonial.rating ? "text-accent" : "text-muted"}>
                    ★
                  </span>)}
              </div>
