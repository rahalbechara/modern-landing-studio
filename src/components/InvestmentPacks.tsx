...
const includes = ["Achat + carte grise", "Assurance + entretien", "Branding + lancement digital", "Possibilité de gestion complète"];
  ...
  return <section className="py-24 bg-background">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto mb-12">
          {packs.map((pack, index) => <Card key={index} className={`p-8 hover:shadow-xl transition-all duration-300 animate-in fade-in slide-in-from-bottom duration-700 ${pack.featured ? 'border-2 border-primary shadow-lg scale-105' : ''}`} style={{
          animationDelay: `${index * 150}ms`
        }}>
              {pack.featured && <div className="inline-block px-4 py-1 bg-primary text-primary-foreground rounded-full text-sm font-semibold mb-4">
                  Plus populaire
                </div>}
        ...
        {/* Includes */}
        <Card className="p-8 max-w-4xl mx-auto bg-gray-900">
          <h3 className="font-bold text-xl mb-6 text-center">Inclus dans les packs :</h3>
          <div className="grid md:grid-cols-2 gap-4">
            {includes.map((item, index) => <div key={index} className="flex items-center gap-3 animate-in fade-in slide-in-from-left duration-700" style={{
